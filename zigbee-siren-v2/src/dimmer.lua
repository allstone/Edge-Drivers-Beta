...
local color_Change_Mode = capabilities["numberreason51831.sirenChangeMode"]
...
function driver_handler.do_init (self, device)
  ...
  if device.preferences.logDebugPrint == true then
    ...
    print("flashingIsOn=",flashingIsOn[id])
  end
 --- ReStart Timer color chanaging function
  ...
  if device:get_field("colorChangeModeList") == nil then
    if device.preferences.logDebugPrint == true then
      print("<< colorChangeModeList nil reseting to Random >>")
    end
    device:set_field("colorChangeModeList", "Random", {persist = true})
    device:emit_event(color_Change_Mode.sirenChangeMode("Random"))
  end
  -- Defaulting - if wrong value then presentastion view UI wont initialize and wont let change attribute
  -- device:emit_event(color_Change_Mode.sirenChangeMode("Random"))
  ...
  if colorChanging == "Inactive" then
    ...
    elseif colorChangeModeList == "AllModes" then
      ...
      elseif colorChangeModeList == "Continuouspurplered" then
        hueMin[device] = 193
        hueMax[device] = 255
      elseif colorChangeModeList == "Flashing" then
        flashingIsOn[device] = false
      end
    end
   ...
   device.thread:call_on_schedule(
    ...
    -- Emit command and color event
      ...
      if colorChangeModeList == "Flashing" then
        if flashingIsOn[device] == false then
          flashingIsOn[device] = true
      ...
      if not colorChangeModeList == "Flashing" then
...
function driver_handler.color_Change_Mode_handler(_,device,command)
  ...
  device:emit_event(color_Change_Mode.sirenChangeMode(colorChangeModeList))
